...
Вихідний код (.arx)
   ...
   Parser.cs       рекурсивний спуск -> AST
      │
   Compiler.cs     AST -> байткод (59 опкодів)
      │
...
23 тести: рекурсія, замикання, фрейми, мапи, методи, стандартна бібліотека,
`try/catch`, модулі, самохостинг, `break`/`continue`, рівність чисел, умови
з дужками, виклик результату виклику напряму. Графічні тести пропускаються
автоматично — вони відкривають вікна.
