...
38 тестів: рекурсія, замикання, фрейми, мапи, методи, стандартна бібліотека,
`try/catch`, модулі (звичайний і вибірковий import), самохостинг,
...
GC-інструментарій, JIT (побайтова ідентичність з ARX_JIT=0/1), вбудована БД
(ArxDb). Графічні тести й HTTP-сервер пропускаються автоматично — вони
відкривають вікна/слухають порт назавжди.
