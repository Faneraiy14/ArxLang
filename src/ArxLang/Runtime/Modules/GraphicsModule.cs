...
// у який ArxLang-скрипт малює. Ввід (клавіатура/миша) накопичується в
// полях канви й опитується нативними функціями (isKeyDown/getMouseX/...).
public class ArxCanvas
{
...
public static class GraphicsModule
{
    public static void Register(Dictionary<string, Func<object[], object?>> registry)
    {
        ...
        registry["canvasShouldClose"] = args => ((ArxCanvas)args[0]).Form.IsDisposed;
        ...
        // екрана. Обертання/математику 3D сам ArxLang-скрипт може робити
        ...
        registry["isMouseDown"] = args => ((ArxCanvas)args[0]).MouseDown;
        registry["getMouseX"] = args => (double)((ArxCanvas)args[0]).MouseX;
        registry["getMouseY"] = args => (double)((ArxCanvas)args[0]).MouseY;
    }
